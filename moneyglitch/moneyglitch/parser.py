...
from .mexc import MEXCError, MEXCFutures, format_diagnostics_html
...
async def _trade_for_account(
    ...
    try:
        result = await mexc.open_long_market(
        ...
        await asyncio.gather(*(
            notify(bot_token, uid,
                   f"❌ <code>{html.escape(account.name)}</code> · "
                   f"MEXC: <code>{html.escape(str(e))}</code>")
            for uid in account.user_ids
        ))
        try:
            diag = await mexc.gather_diagnostics(account.symbol)
    ...
    set_position(account.name, {
        ...
        "neutral_tp_price": result.get("neutral_tp_price"),
        "neutral_tp_qty": result.get("neutral_tp_qty"),
        "neutral_tp_vol": result.get("neutral_tp_vol"),
...
async def run_parser(config: Dict[str, Any]) -> None:
    ...
    async def handler(event):
        ...
        async with trade_lock:
            ...
            await asyncio.gather(*(
                _trade_for_account(
                    a, mexc_clients[a.name], bot_token, msg.id, snippet, ai_result,
                )
                for a in accounts
    ...
    try:
        ...
        await asyncio.gather(*(c.aclose() for c in mexc_clients.values()), return_exceptions=True)
