...
AI_CONFIG_PATH = Path(os.environ.get("MONEYGLITCH_AI_CONFIG") or "ai_config.json")
...
def load_ai_config() -> Dict[str, Any]:
    ...
    file is missing/unreadable — `ai_config_status()` exposes the specific
    reason for diagnostic display in the bot.
    ...
    try:
        ...
        log.warning("ai_config at %s unreadable: %s", AI_CONFIG_PATH, e)
        return {}
    if not isinstance(data, dict):
        log.warning("ai_config at %s is not a JSON object", AI_CONFIG_PATH)
        return {}
    return data
...
def ai_config_ready(cfg: Dict[str, Any]) -> bool:
    ...
    return all(bool(cfg.get(k)) for k in REQUIRED_AI_FIELDS)


def ai_config_status() -> Dict[str, Any]:
    """Self-diagnostic for the bot UI. Tells the user *why* the global AI
    config is or isn't usable. Independent of `load_ai_config` so the bot
    can show a meaningful message without changing the parser contract.

    Returns one of:
        {"state": "ready",       "cfg": dict}
        {"state": "missing"}                          — file not found
        {"state": "unreadable",  "reason": str}       — IO or permission error
        {"state": "invalid_json","reason": str}
        {"state": "not_object"}                        — JSON parsed but not a dict
        {"state": "incomplete",  "missing": [str], "cfg": dict}
    """
    if not AI_CONFIG_PATH.exists():
        return {"state": "missing"}
    try:
        raw = AI_CONFIG_PATH.read_text(encoding="utf-8-sig")
    except (OSError, UnicodeDecodeError) as e:
        return {"state": "unreadable", "reason": str(e)}
    try:
        data = json.loads(raw)
    except json.JSONDecodeError as e:
        return {"state": "invalid_json", "reason": str(e)}
    if not isinstance(data, dict):
        return {"state": "not_object"}
    missing = [k for k in REQUIRED_AI_FIELDS if not data.get(k)]
    if missing:
        return {"state": "incomplete", "missing": missing, "cfg": data}
    return {"state": "ready", "cfg": data}
