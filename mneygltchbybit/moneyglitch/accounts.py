...
An account = one set of Bybit credentials + the Telegram user IDs allowed
...
def load_accounts(config: Dict[str, Any]) -> List[AccountConfig]:
    ...
    (`bybit` + `bot.user_ids`) so old configs still work."""
    ...
    if not raw:
        ...
        if not bybit.get("api_key"):
            raise ValueError("config has neither 'accounts' nor 'bybit'")
        raw = [{"name": "main", "user_ids": users, "bybit": bybit}]
    ...
    for i, a in enumerate(raw):
        ...
        for u in users:
            ...
            seen_users[u] = name
        bybit = a.get("bybit") or {}
        if not bybit.get("api_key") or not bybit.get("secret"):
            raise ValueError(f"account '{name}' missing bybit api_key/secret")
        accounts.append(AccountConfig(
            name=name,
            user_ids=users,
            api_key=str(bybit["api_key"]),
            secret=str(bybit["secret"]),
            symbol=str(bybit.get("symbol") or "TONUSDT"),
            isolated=bool(bybit.get("isolated", True)),
        ))
    return accounts
