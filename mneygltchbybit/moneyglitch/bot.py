"""Telegram control bot. Russian UI. Owner-gated by config bot.user_ids."""
...
from .parser import normalize_user_ids
...
def build_dispatcher(owner_ids: Iterable[int]) -> Dispatcher:
    dp = Dispatcher(storage=MemoryStorage())
    owners: FrozenSet[int] = frozenset(int(x) for x in owner_ids)

    def is_owner_msg(m: Message) -> bool:
        return m.from_user is not None and m.from_user.id in owners

    def is_owner_cb(q: CallbackQuery) -> bool:
        return q.from_user is not None and q.from_user.id in owners
...
async def run_bot(config: Dict[str, Any]) -> None:
    ...
    owners = normalize_user_ids(bot_cfg)
    if not owners:
        raise RuntimeError("config.bot.user_ids is empty — at least one owner is required")
    dp = build_dispatcher(owners)
    log.info("bot polling started; owners=%s", owners)
